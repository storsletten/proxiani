...
const gc = (data, middleware, linkedMiddleware) => {
 ...
 if (data.command.length > 1) {
  ...
  if (data.command.length === 2) {
   if (data.command[1] in knownSectors) {
    gcTarget = knownSectors[data.command[1]];
    sector = `Sector ${data.command[1]}`;
   }
   else return;
  }
  else if (data.command.length > 3) {
   const m = data.input.match(/(-?[0-9]{1,10}),? (-?[0-9]{1,10}),? (-?[0-9]{1,10})/);
   if (m) gcTarget = m.slice(1, 4).map(n => Number(n));
  }
